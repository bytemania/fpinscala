...
case class Prop(run: (MaxSize,TestCases,RNG) => Result) {
  def &&(p: Prop) = Prop {
    (max,n,rng) => run(max,n,rng) match {
      case Right((_,_)) => p.run(max,n,rng).right.map { case (s,m) => (s,n+m) }
      case l => l
    }
  }
  def ||(p: Prop) = Prop {
      ...
      def go(i: Int, j: Int, s: Stream[Option[A]], onEnd: Int => Result): Result =
        if (i == j) Right((Unfalsified, i))
          ...
          case Cons(h,t) => h() match {
            case Some(h1) =>
              try {
                if (f(h1)) go(i+1,j,t(),onEnd)
                else Left(h1.toString) }
              catch { case e: Exception => Left(buildMsg(h1, e)) }
